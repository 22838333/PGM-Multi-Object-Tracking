...
int main(int argc, char *argv[]) {
  try {
    ...
    vector<double> noiseXList1;
    vector<double> noiseYList1;
    vector<double> noiseXList2;
    vector<double> noiseYList2;
    ...
    while (inFile >> c) {
      switch(c) {
        case '[':
          counter++;
          if(OL >= Verbose) cout << "start x parse" << counter << endl;
          break;
        case ']':
        case ',':
          if(counter == 1){
            ...
            noiseXList1.push_back(std::stod(str));
            str.clear();
          }else if(counter == 3){
            noiseXList2.push_back(std::stod(str));
            str.clear();
          }
    ...
    while (inFile >> c) {
      switch(c) {
        case '[':
          counter++;
          if(OL >= Verbose) cout << "start Y parse " << counter << endl;
          break;
        case ',':
        case ']':
          if(counter == 1){
            ...
            noiseYList1.push_back(std::stod(str));
            str.clear();
          }else if(counter == 3){
            noiseYList2.push_back(std::stod(str));
            str.clear();
          }
          break;
        case ' ':
    ...
    for(uint t = 0; t<timesteps-1; t++){
      ...
      for(uint o = 0; o<objects; o++){
        ...
        for(uint m = 0; m<measurements;m++){
          ...
          if(m == 0){
            mn[0] = measXList[t]; mn[1] = measYList[t];
          } else if(m == 1){
            mn[0] = noiseXList1[t]; mn[1] = noiseYList1[t];
          } else if(m == 2){
            mn[0] = noiseXList2[t]; mn[1] = noiseYList2[t];
          }
      ...
      for(uint o = 0; o<objects; o++){
        ...
        for(uint m = 0; m<measurements;m++){
          ...
          if(m == 0){
            ...
            mn[0] = noiseXList1[timesteps-1]; mn[1] = noiseYList1[timesteps-1];
          } else if(m == 2){
            mn[0] = noiseXList2[timesteps-1]; mn[1] = noiseYList2[timesteps-1];
          }
          cov(0,0) = 10.0; cov(0,1) = 0.0;
          cov(1,0) = 0.0; cov(1,1) = 10.0;
          map< vector<T>, tuple< double, prlite::ColVector<double>, prlite::RowMatrix<double> > > weightedComponents =
        ...
        mnEx[0] = 50-25*o; mnEx[1] = 50-25*o; mnEx[2] = 0; mnEx[3] = 0; mnEx[4] = 0; mnEx[5] = 0;
